def test_lookup_returns_english_gloss(dictionary):
    ...
    glosses = [str(g) for entry in result.entries for sense in entry.senses for g in sense.gloss]
    assert any("cat" in g.lower() for g in glosses)

    result2 = dictionary.lookup("食べる")
    glosses2 = [str(g) for entry in result2.entries for sense in entry.senses for g in sense.gloss]
